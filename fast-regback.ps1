...
if ($Uninstall) {
    # найден аргумент -uninstall
    if (Test-Path -Path $destPath) {
        Remove-Item -Path $destPath -Recurse -Force
        Write-Output "Directory removed."
    } else {
        Write-Output "Directory does not exist: $destPath"
    }
# установка бэкапа, -uninstall отсутствует
} else {
    ...
    if (-Not (Test-Path -Path $destPath\regbak-desktop.ini)) {
        ...
        $shortcut.Save()
        New-Item -Path $destPath -Name "regbak-desktop.ini" -ItemType "file" > $null
        }
}
